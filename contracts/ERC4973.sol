...
abstract contract ERC4973 is ERC165, IERC721Metadata, IERC4973 {
  ...
  mapping(address => uint) private _tokenCheck;
  ...
  function tokenUsernames(uint256 tokenId) public view virtual returns (string memory) {
    require(_exists(tokenId), "tokenUsernames: token doesn't exist");
    return _tokenUsernames[tokenId];  
  }

  function tokenBio(uint256 tokenId) public view virtual returns (string memory) {
    require(_exists(tokenId), "tokenBio: token doesn't exist");
  ...
  function getTokenId(address _address) public view virtual returns (uint) {
    return _tokenCheck[_address];
  }
